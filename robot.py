...
The overall strategy at this point is to glom together a group big enough to take the center, then fight defensively to annoy the opponent to death. The advanced movement rules allow for strong offensive play by the smaller gloms, as well.
...
def init_turn(state):
    ...
    if not len(enemies) or len(enemies) < 2:
