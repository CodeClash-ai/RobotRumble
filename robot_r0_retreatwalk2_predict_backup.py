from typing import *
# Bot vs mitch84__retreat_walk2: opponent RETREATS (moves to first blank N,S,E,W) when
# (>1 of our units adjacent) OR (exactly 1 of our units adjacent AND that unit's health
# > its own health). Movement resolves BEFORE attacks, so attacking a fleeing unit MISSES.
# Counter: PREDICT retreat (both cases!) -> attack the flee-TO tile / target stayers;
# retreat_tiles model the flee tiles so we TRAP (block them). Focus-fire + cautious advance.
...
def init_turn(state):
    ...
    retreat_tiles=set()
    for e in E:
        # will e retreat? (>1 our unit adjacent) OR (1 our unit adjacent stronger than e)
        ours=[]
        for d in DIRECTIONS:
            o=state.obj_by_coords(e.coords+d)
            if o is not None and o.obj_type==ObjType.Unit and o.team==state.our_team: ours.append(o)
        will_flee = (len(ours)>1) or (len(ours)==1 and ours[0].health>e.health)
        if will_flee:
            for d in DIRECTIONS:
                c=e.coords+d
...
def our_adj_units(state,c):
    r=[]
    for d in DIRECTIONS:
        o=state.obj_by_coords(c+d)
        if o is not None and o.obj_type==ObjType.Unit and o.team==state.our_team: r.append(o)
    return r
def first_blank(state,e):
    for d in DIRECTIONS:
        c=e.coords+d
        if inb(c) and state.obj_by_coords(c) is None:
            return c
...
def predict_retreat(state,e):
    """Model opponent retreat_walk2: it retreats (moves to first blank N,S,E,W) if
    (>1 of our units adjacent) OR (exactly 1 of our units adjacent AND that unit's
    health > its health), provided a blank adjacent tile exists. Return flee-to coords or None."""
    ours=our_adj_units(state,e.coords)
    n=len(ours)
    if n>1:
        return first_blank(state,e)
    if n==1 and ours[0].health>e.health:
        return first_blank(state,e)
    return None
def robot(state,unit):
    ...
    if adj:
        ...
        if unit.health<=3 and not can_kill and stayers:
            best=None
    ...
    for d in DIRECTIONS:
        ...
        # kill-approach: moving adjacent to an enemy that then becomes killable
        killapp=1
        for dd in DIRECTIONS:
            ec=nc+dd
            if (ec.x,ec.y) in enemy_set:
                eo=state.obj_by_coords(ec)
                if eo is not None and eo.obj_type==ObjType.Unit and eo.team==state.other_team:
                    if attackers_on.get((ec.x,ec.y),0)+1>=eo.health:
                        killapp=0
        dist=nc.walking_distance_to(tgt.coords)
        cdist=abs(nc.x-ally_cx)+abs(nc.y-ally_cy)
        best_key=(trap,killapp,dist,cdist,th,diag,th-su)
