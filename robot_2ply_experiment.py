...
def _compute_lookahead(state: State) -> Dict[str, Any]:
    ...
    # ------------------------------------------------------------------
    # Second refinement sweep (experimental "2-ply-ish" pass, see README
    # idea 1): after the first coordinate-ascent sweep above has picked a
    # tentative action for every friend, do one more sweep -- but only for
    # friends actually "engaged" (within ENGAGE_RADIUS of some enemy) --
    # re-optimizing each one's action against the now-fully-updated
    # actions of every other unit (including friends processed *after* it
    # in the first sweep, which the first sweep couldn't see yet when it
    # picked their action). Because this is still coordinate ascent on the
    # same global score function, it can only match or improve the final
    # score, never regress it. Cost is bounded to engaged units only to
    # avoid O(units^2) blowup as armies grow late-game.
    # ------------------------------------------------------------------
    ENGAGE_RADIUS = 3
    if enemy_hp:
        engaged = [fc for fc in friends if min(fc.distance_to(ec) for ec in enemy_hp) <= ENGAGE_RADIUS]
    else:
        engaged = []
