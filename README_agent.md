...
Round 1 current note (gpt-5-5 vs mitch84__crw_preempt):
- Reviewed `/logs/rounds/0`: our bot was Red and scored 246 wins / 4 losses. The four Blue wins were `sim_43`, `sim_54`, `sim_103`, and `sim_170`; all were already Red deficits after the final clear/spawn (e.g. sim_43 Blue 28 vs Red 25 after turn 90; sim_54 29 vs 26; sim_103 tied at 27 then Red fell behind; sim_170 28 vs 27).
- Made a narrow late-comeback tweak in `robot.py`: when behind after turn 91 and the existing wounded-target `late_desperation_step()` has no candidate, use bounded `late_pressure_step()` only for larger deficits (down 3+ immediately after turn 91, or down 2+ from turn 95). This intentionally does not affect one-unit deficits or any lead/tie preservation, because prior generic late pressure regressed against line/cluster bots.
- Smoke tests after the tweak still pass: passive seed 43 loses as both colors (39-4 units), naive nearest-chaser seed 43 loses as both colors (our bot 34-9 as Blue, 33-9 as Red), and self-play seeds 43/54 are valid in ~5-6s. Future validation should focus on whether the four logged final-wave deficit losses improve; if new late over-chasing appears, revert only these two `late_pressure_step()` fallback call sites.

Round 2 current note (gpt-5-5 vs mitch84__crw_preempt follow-up):
- Reviewed `/logs/rounds/0` and `/logs/rounds/1`: r0 was 246W/4L as Red; r1 after the prior multi-unit late-pressure fallback was 247W/2L/1T as Blue. Remaining r1 non-wins were `sim_43` (tied 27-27 after final spawn with Blue only -2 health, then late trades collapsed to 23-26), `sim_133` (27-27 tie with Blue -4 health and no unit changes after turn 90), and `sim_205` (Blue down 29-31 after final spawn and never recovered).
- Made two narrow final-wave tweaks in `robot.py`: (1) when unit counts are tied from turn >=90 and we are only slightly behind on health (-1 to -5), stop the wounded-target chase and instead kite/intercept/hold; adjacent attacks in that band require a focused kill that should survive. This targets sim_43-style conversion of a draw into a loss, while accepting sim_133 may remain a draw. (2) When behind by 2+ after turn 91, allow the bounded `late_pressure_step()` immediately after wounded-target desperation fails (previously down-2 waited until turn 95). This targets sim_205-style multi-unit final-wave deficits; one-unit deficits still avoid broad pressure.
- Smoke tests after edits: passive still loses hard (seed 43 Blue 39-4), naive nearest-chaser still loses as both colors on seed 43 (34-9 / 33-9), seed 205 vs chaser wins (35-6 as Blue), and self-play seed 2 remains valid in ~5.7s. Watch future logs for any over-passive tied small-health-deficit draws or over-chasing when down exactly two after the final spawn.

Round 1 current note (gpt-5-5 vs entropicdrifter__glommer):
- Reviewed `/logs/rounds/0`: current bot was Blue and scored 205W / 12L / 33T. This is a much stronger mirror/line-style opponent than the passive bots; many non-wins are late exact unit-count ties where Blue has a sizeable health edge (e.g. sim_0 tied 35-35 with +42 HP) or late deficits after the final spawn (worst sim_25 ended 23-31).
- Made a narrow late tie-pressure tweak in `robot.py`: added `late_health_pressure_step()` and use it only after the final spawn in tied/behind states when our HP buffer is meaningful. In exact ties with modest HP edge, turns >=97 now try wounded-target pressure and then bounded weak-target pressure after safer kite/intercept options fail. In ties/deficits with >=25 or >=15 HP edge, respectively, bounded HP pressure can also run from turn >=94. The spawn-evacuation/annulus macro is unchanged.
- Local validation: new bot vs previous copy improved self-play seed 1 from tie to Blue win (35-34) and was identical/comparable on seeds 2-5 (including the existing seed-4 Red win). Reverse color early seeds were unchanged. Smoke tests still pass versus passive (seed 1 Blue 39-4) and naive nearest-chaser (seed 2 Blue 27-9). Watch future logs for over-pressure in late tied states; if it regresses, revert the `late_health_pressure_step` call sites.

Round 2 current note (gpt-5-5 vs entropicdrifter__glommer follow-up):
- Reviewed `/logs/rounds/0` and `/logs/rounds/1`: r0 was 205W/12L/33T as Blue; r1 after the prior HP-pressure tweak improved to 217W/16L/17T as Red. This is a strong mirror/line-style opponent; many non-wins remain late exact ties/losses, often with our side ahead on total HP but tied on units after the final spawn.
- Made one narrow late-game combat tweak in `robot.py`: when unit counts are exactly tied from turn >=90 and we have any positive health edge, adjacent combat now requires a focused kill that should leave the attacker alive; otherwise the unit retreats/holds (unless trapped and likely dying anyway, where a focused attack is allowed). This aims to preserve HP-edge ties instead of converting them into unit losses while the existing bounded late pressure seeks safe kills. Pre-final-wave spawn evacuation and annulus macro are unchanged.
- Smoke tests after edit still pass: passive seed 2 loses as both colors (36-4 units), naive nearest-chaser seed 2 loses as both colors (our bot 27-9 as Blue, 30-5 as Red). Mirror/new-vs-previous spot checks were noisy/mixed (some early seeds unchanged, seed 2 new Blue gained units, some close seeds still poor), so future validation should focus on r2 logs and turns 90-100; if this over-preserves and increases ties, revert only the new `health_edge > 0` adjacent-combat block.

Round 1 current note (gpt-5-5 vs mousetail__coward-bot):
- Reviewed `/logs/rounds/0`: our current bot was Red and scored 230W / 15L / 5T. This opponent is much more competitive than passive bots but still usually loses to our spawn-evacuation/annulus macro. Non-wins are concentrated in late/final-wave situations; many Blue wins were already Blue-favored after turn 90, while ties (e.g. sims 29/81/165/211/246) were exact unit-count draws with small health edges and little/no movement after the final spawn.
- Made two narrow late-game tweaks in `robot.py`: (1) in exact unit ties with only a tiny health deficit (-1/-2 HP), after safer kite/intercept options fail, allow the existing wounded-target nudge from turn >=98 to try converting sterile draws into +1 wins; (2) if behind from turn >=80, try the bounded wounded-target desperation nudge after required wall/spawn-safety movement. This only targets nearby wounded non-spawn enemies and leaves the core spawn evacuation/annulus strategy unchanged.
- Smoke tests after edit still pass: passive seed 1 loses 39-4; naive nearest-chaser seed 2 loses as both colors (our Blue 34-7, our Red 30-9). Mirror checks are noisy; future validation should focus on the logged non-wins above. If the turn>=80 wounded nudge creates over-chasing, revert just that inserted post-wall block first.

Round 2 current note (gpt-5-5 vs mousetail__coward-bot follow-up):
- Reviewed `/logs/rounds/0` and `/logs/rounds/1`: r0 (our Red) was 230W/15L/5T, r1 (our Blue) was 225W/16L/9T. Non-wins in r1 were mostly final-wave unit-count ties/losses; many losses were already tied/behind around turns 85-90, while several ties froze with no unit changes after turn 90.
- Made one conservative revert-like tweak in `robot.py`: removed the previous turn>=80 pre-final wounded-target desperation nudge. The next logged round still had many late losses/ties, and seed timelines suggest pre-final chasing may bleed bodies before the final spawn/clear. Pre-final behavior is now survival-first again; existing turn>=91 final-wave desperation/pressure logic remains unchanged.
- Smoke tests after the tweak: passive still loses as both colors (seed 2: 36-4 units either side), naive nearest-chaser still loses as both colors (seed 2: our Blue 34-7, our Red 30-9), and self-play seed 2 is valid (~5.6s). Future validation should check whether this improves the current coward-bot r1 non-wins; if ties persist, focus narrowly on turn 90-100 exact-tie pressure rather than reintroducing pre-final chasing.

Round 1 current note (gpt-5-5 vs entropicdrifter__glommerv2):
- Reviewed `/logs/rounds/0`: current bot was Blue and only scored 110W / 103L / 37T against a strong mirror/line-style Red. `python3 analyze_logs.py` shows averages Blue 35.02 units vs Red 35.59; many games are decided entirely in turns 90-100.
- Turn timeline analysis showed positions tied on units after the final spawn mostly stayed ties or became losses; they did not convert into wins. I made one conservative late-game change in `robot.py`: from turn >=90, when unit counts are exactly tied, units now only kite nearby threats / pre-fire intercept squares and otherwise hold, instead of using wounded/health-pressure movement toward enemies. Adjacent tied combat logic is unchanged, and all pre-final-wave spawn evacuation/annulus macro is unchanged.
- Smoke tests still pass: passive seed 1 loses 39-4, naive nearest-chaser seed 2 loses as both colors (our Blue 27-9, our Red 30-5). New-vs-previous mirror spot checks became more drawish/mixed, so future validation should check whether this reduces entropicdrifter tie-to-loss bleed; if it over-preserves and increases draws too much, revert the new tied-state block around `robot.py` line ~490.

Round 2 current note (gpt-5-5 vs entropicdrifter__glommerv2, this session):
- Reviewed `/logs/rounds/0` and `/logs/rounds/1`: r0 as Blue was 110W/103L/37T, r1 as Red improved to 113W/87L/50T after the prior tied-state preservation change. This opponent remains very strong/mirror-like; many non-wins are decided in the final wave, with numerous exact unit-count ties where our side has a large health edge but no units die after turn 90.
- Made one narrow late tie tweak in `robot.py`: in the turn>=90 exact-unit-tie preservation block, after kiting and intercept pre-fire fail, robots with a sizeable health edge (>=20 HP) may use `late_health_pressure_step()` from turn>=97. This keeps the conservative tie-preservation first, but tries to convert sterile high-HP final ties into +1 unit wins without reintroducing broad generic late pressure.
- Smoke tests after the tweak: self-play seeds 1-3 stayed valid/comparable (ties on 1/2, Blue +1 on 3), passive still loses as both colors on seed 2 (36-4), and naive nearest-chaser still loses as both colors on seed 2 (our bot 30-9 as Blue, 30-7 as Red). If future logs show over-pressure late losses, revert only this small `health_edge >= 20` turn>=97 block around `robot.py` line ~512.

Round 1 current note (gpt-5-5 vs entropicdrifter__we-are-borg):
- Reviewed `/logs/rounds/0`: our bot was Blue and won the match overall but only scored 116W / 87L / 47T against a very strong mirror/Borg-style Red. Averages were essentially even on units (Blue 35.33 vs Red 35.57) with our side ahead on HP (163.18 vs 154.65); many non-wins are final-wave exact unit ties where Blue has a modest HP edge but cannot convert, or late deficits already present after turn 90.
- Added `analyze_borg.py` for turn-by-turn unit/HP summaries. Useful findings: final ties averaged +11.5 Blue HP, with 28/47 final ties at +12 HP or better; Blue wins usually have much larger HP edge (avg +22.9), while Red wins often involve us bleeding tied/+HP positions into -1 losses late.
- Made one narrow late tie-pressure tweak in `robot.py`: in the existing turn>=90 exact-unit-tie preservation block, after kiting/intercept fail, lower the turn>=97 `late_health_pressure_step()` threshold from `health_edge >= 20` to `>= 12`. The goal is to convert sterile +12..+19 HP final ties into +1 wins without changing spawn evacuation, annulus macro, lead preservation, or behind-state desperation.
- Smoke tests after the tweak still passed versus passive/chaser as both colors on seed 2. New-vs-previous mirror spot checks are noisy: seeds 1/2 tied, seed 3 Blue +1, seed 10 Red +1 (same as old-vs-old). If next logs show more tied-state overpressure losses, revert only the threshold at `robot.py` line ~512 back to 20; otherwise consider further tuning turns 97-100 exact ties.

Round 2 current note (gpt-5-5 vs entropicdrifter__we-are-borg follow-up):
- Reviewed `/logs/rounds/0` and `/logs/rounds/1`: r0 as Blue was 116W/87L/47T, r1 as Red after lowering exact-tie HP-pressure threshold to 12 was 101W/91L/58T. This Borg/mirror-style opponent remains very close; many non-wins are final-wave exact unit ties with our side ahead on HP, while losses are usually already unit deficits after the final spawn.
- Made one narrow late tie-pressure tweak in `robot.py`: in the turn>=90 exact-unit-tie preservation block, the bounded `late_health_pressure_step()` for HP-edge ties now starts at turn>=94 instead of turn>=97 (threshold still `health_edge >= 12`, radius 7). Kiting/intercept still run first, and lead/behind/pre-final spawn behavior is unchanged. The intent is to give sterile +12 HP ties (common in logs) a few extra turns to convert to +1 without generic chasing.
- Smoke tests after edit still pass: passive seed 2 loses 36-4, naive chaser seed 2 loses with our bot as Blue (27-9) and as Red (30-5), and self-play seed 1 remains a valid 35-35 tie in ~6s. If future logs show over-pressure late losses, revert only this `state.turn >= 94` threshold back to 97 around `robot.py` line ~512.

Round 1 current note (gpt-5-5 vs entropicdrifter__seven-of-nine):
- Reviewed `/logs/rounds/0`: our bot was Red and is now behind this very strong Borg/mirror-style Blue, scoring 75W / 129L / 46T. Red averages 30.58 units vs Blue 32.22; many non-wins are already Blue-favored after the final spawn, but several ties/losses show Red had a +1/+2 lead around turns 95-99 and then bled it away.
- Made one narrow late-preservation tweak in `robot.py`: while ahead from turn >=95, adjacent focused kills now require the attacker to likely survive local adjacent focus (`unit.health > len(adj)`) for both interior and final-wave spawn/perimeter robots. Previously this stricter rule only applied to 2+ leads (or turn 99+), which allowed one-unit leads to trade down in the final stretch.
- Smoke tests after edit: passive still loses hard (seed 2 Blue 36-4); naive nearest-chaser still loses as both colors (seed 2: our Blue 34-7, our Red 30-9). New-vs-previous mirror seeds 1/2 were unchanged ties; a few oldBlue-newRed spot checks were mixed/noisy. Future work should focus on turns 90-100 of Red non-wins; most large Blue wins are already decided before the final wave, so avoid broad macro/chase changes unless logs justify them.

Round 2 current note (gpt-5-5 vs entropicdrifter__seven-of-nine follow-up):
- Reviewed `/logs/rounds/0` and `/logs/rounds/1`: this is a very strong Borg/mirror-style opponent. Round 0 as Red was 75W/129L/46T; round 1 as Blue after the prior one-unit-lead preservation improved slightly to 77W/107L/66T. Many non-wins are final-wave exact unit ties or small deficits; round 1 had 66 ties, with many tied games ending with our HP edge but no unit conversion.
- Made two narrow turn-90+ tie-state tweaks in `robot.py`: (1) exact unit-count ties with adjacent enemies now always require a focused kill that should leave the attacker alive, regardless of health edge; otherwise retreat/hold, reducing tie-to-loss adjacent trades. (2) in the non-adjacent tied-unit preservation block, after kiting/intercept fail, bounded `late_health_pressure_step()` now starts at health edge >=5 (was >=12) from turn >=94. This aims to convert more sterile +5..+11 HP final ties while still preserving/kiting first. Pre-final spawn evacuation and the annulus macro are unchanged.
- Smoke tests after edit: passive and naive nearest-chaser still lose decisively (seed 2 Blue: passive 36-4 units; chaser 30-9 units), and self-play seed 1 remains valid/tied in ~6.4s. Mirror/new-vs-previous checks on seeds 1-4 were mostly identical/no obvious regression, but future validation should focus on actual logs; if new late over-pressure losses appear, first revert the threshold at `robot.py` line ~486 back from `health_edge >= 5` to `>= 12`.

Round 3 current note (gpt-5-5 vs entropicdrifter__seven-of-nine):
- Reviewed `/logs/rounds/0..2`: this very strong Borg/mirror-style opponent is beating us overall. Scores moved from 75W/129L/46T (r0, us Red) to 77W/107L/66T (r1, us Blue) to 73W/129L/48T (r2, us Blue). Round 2 regressed after lowering exact-tie HP-pressure to `health_edge >= 5`; many tied +5..+11 HP positions became Red wins rather than ties.
- Reverted that specific exact-unit-tie HP-pressure threshold back to `health_edge >= 12` at turn>=94. This keeps the safer r1-style behavior and avoids over-pressuring small HP edges.
- Added a narrow trapped-adjacent exception in late tied/ahead preservation: if a unit cannot retreat and local focus should kill the adjacent target, it attacks instead of holding. The intent is to avoid dying for free when already adjacent and trapped; pre-final spawn evacuation/annulus macro is unchanged.
- Smoke tests after edits: passive seed 2 still loses 36-4, naive chaser seed 2 still loses as Blue 34-7 and as Red 30-9, and self-play seed 2 remains valid/tied in ~6.4s. Future validation should focus on turn 90-100 exact ties; if trapped-trade creates new late bleed, revert only those three small fallback attack blocks.

Round 4 current note (gpt-5-5 vs entropicdrifter__seven-of-nine):
- Reviewed `/logs/rounds/0..3`: opponent remains a very strong Borg/mirror-style bot and is ahead overall. Latest r3 (our Blue) was 76W / 131L / 43T; many non-wins are final-wave exact ties or small deficits, and r3 regressed slightly after the prior trapped-adjacent trade exception.
- Made two focused changes in `robot.py`: (1) removed the late trapped/focused-trade fallback in exact-tie/ahead preservation blocks, so if a unit cannot retreat it holds instead of deliberately trading down a tied/lead state; (2) shifted the defensive annulus preference from radius ~7 to ~8 (comments/score functions only; center unclustering threshold left at `<6`). Local new-vs-previous mirror spot checks were noisy but radius-8 won several seeds as Blue and remained valid; simple passive/chaser smoke tests still win decisively.
- Smoke tests after edits: passive seed 2 loses 36-4, naive chaser seed 2 loses 36-4, self-play seed 2 valid tie in ~6s. Future validation should focus on r4 logs against seven-of-nine. If radius-8 hurts, revert the three `- 8` annulus scoring constants/comments back to `- 7`; if late ties still bleed, inspect turns 95-100 before reintroducing any trapped trade logic.

Round 5 current note (gpt-5-5 vs entropicdrifter__seven-of-nine final):
- Reviewed `/logs/rounds/0..4` with `python3 analyze_logs.py`. This Borg/mirror-style opponent remains ahead; the best logged result was round 1 (77W/107L/66T as Blue). Later changes regressed: r2 73W/129L/48T, r3 76W/131L/43T, r4 73W/140L/37T.
- Reverted `robot.py` to the round-1 code (`git checkout 1efda4e -- robot.py`): defensive annulus back to radius ~7, exact-tie adjacent combat back to the earlier health-edge-specific handling, and removed the later radius-8/trapped-trade experiments. This preserves the best observed code for this opponent.
- Validation after revert: `python3 -m py_compile robot.py`, self-play seeds 1/2 valid ties in ~6-7s, passive seed 2 still loses as both colors (36-4 units). Submit this safer best-logged version unless future logs show a clearly better late-game tweak.
