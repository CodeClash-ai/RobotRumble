...
## Round 23 (this session — starting point was fresh /logs/rounds/0/ only, will produce /logs/rounds/1/)

**Status check (first thing, per standing advice, 23rd time):** Only
`/logs/rounds/0/results.json` was present at session start. Opponent this
series is `navster8__maginot-line` (14th differently-named opponent
identity across the rounds documented in this file). Result: **250/250
sweep for sonnet-5** (sonnet-5 was Blue; `Blue wins 250, Red wins 0, ties
0` via the standard win/loss snippet). Avg final units: us (Blue) ~29.83
(min 16, max 40), opponent (Red) ~2.01. Twenty-third consecutive total
sweep documented in this file (across 14 differently-named opponent
identities so far). Grepped all 250 `sim_*.txt` for
`raceback|xception|panic` — zero hits, confirming clean execution across
the entire series. Margin (~14.8x) is back on the high end of the
historical range (the last few rounds vs `kalkin__artemis`/`kalkin__artemis2`
had settled around ~4.6-5.6x; this round's opponent looks weaker again,
consistent with normal opponent-to-opponent variance rather than a trend).

### What I did this round
1. Confirmed `robot.py` has zero diff from `git show HEAD:robot.py`
   (`git status --short` clean at session start) — no drift from the
   version described in rounds 4-22 above (per-unit soft targeting
   blending own-distance + target health + team coordination-distance +
   focus-bonus; opportunistic always-attack-if-adjacent, weakest-first;
   `direction_to` movement w/ full 4-direction sidestep fallback;
   spawn-tile-escape when no enemies visible).
2. Sanity-checked it still runs clean and fast:
   - `./rumblebot run term --results-only robot.py robot.py` — no
     exceptions, real combat (`Health 11 10 Units 3 2`).
   - `./rumblebot run term --results-only robot.py robot_v1_baseline.py`
     — no exceptions, real combat (`Health 13 24 Units 3 5`, baseline won
     this particular single seed — expected occasional variance, see the
     larger sample below).
3. Used the persistent `tools/ab_test.py` harness (from Round 12) to
   re-confirm `robot.py`'s edge over `robot_v1_baseline.py` over 40 seeds,
   both sides (`--swap`): robot.py-as-Blue **26 wins / 12 losses / 2
   ties**; robot.py-as-Red (swapped) **24 wins / 13 losses / 3 ties**.
   Combined across both directions: **50 wins / 25 losses / 5 ties**
   (~66.7% of decisive games) — consistent, side-independent, matches the
   long-running ~55-70% edge over the round-0 baseline reported across
   nearly every previous round (this is actually the exact same 40-seed
   command/result pattern as Round 22's check — reran independently this
   round and got an identical breakdown, which is expected since the A/B
   harness runs are deterministic given the same seeds/bots/no RNG drift
   in either bot file). No regression.
4. **No code changes made this round.** Same reasoning as rounds 2-22: 23
   consecutive 250/250 sweeps across 14 different opponent identities,
   still overwhelming (~14.8x this round) final-unit-count margins, zero
   runtime errors ever observed across ~5750+ simulated games total, and
   every tunable knob in the bot (all 3 weight constants — settled
   no-effect at large sample in Rounds 10 & 12 — overkill-avoidance
   targeting — Round 6, mild negative — BFS-pathing — judged low-value in
   Round 5) has already been explored with no robust improvement found.
   Validation-only round again — no evidence anywhere in 23 rounds that
   the opponents are close to competitive enough to justify the
   regression risk of a large rewrite.

### Suggested next steps for future teammates
- Standing advice unchanged (23rd time writing this): check
  `/logs/rounds/N/results.json` + final-unit-count margins FIRST thing
  next round, before making changes. Use `tools/ab_test.py` (see Round 12
  notes for usage) for any self-play A/B testing rather than recreating a
  script from scratch.
- Opponent-strength margin bounces around a lot by opponent identity
  (~4.6x-15x observed across rounds 19-23) but has never come close to
  threatening our 100% round-win record. Keep tracking it, but don't
  over-react to any single round's number in isolation — look at the
  multi-round trend as previous rounds' notes have done.
- All three tunable weight constants (`HEALTH_WEIGHT`, `FOCUS_BONUS`,
  `COORD_WEIGHT`) remain settled/closed from Rounds 10 & 12's large-sample
  tests (all ~50% i.e. no effect) — do not re-litigate without a
  fundamentally different targeting idea.
- Genuinely still-untried ideas (unchanged across ~23 rounds now):
  multi-step (2-3 move) lookahead pathing for escaping dead-ends near the
  map's wall corners (still low-value per Round 5's analysis); explicit
  "retreat when badly outnumbered locally" logic for individual low-health
  units (still untried, carries real regression risk against the current
  "always attack if adjacent" philosophy that keeps winning decisively).
  Neither has been necessary yet across 23 rounds and 14 opponent
  identities, all crushed by ~4.6x+ unit-count margins minimum.
- `robot_v1_baseline.py` remains the frozen round-0 reference bot for A/B
  self-play testing — do not delete/modify it. Still gives `robot.py` a
  consistent, reproducible, side-independent edge (this round: 50-25-5
...
## Round 24 (this session — starting point had /logs/rounds/0/ AND /logs/rounds/1/
already present, will produce /logs/rounds/2/)

**Status check (first thing, per standing advice, 24th time):** Both
`/logs/rounds/0/results.json` and `/logs/rounds/1/results.json` were
already present at session start. Opponent this series is
`navster8__maginot-line` (same name as round 23's notes above — round 0
and round 1 here are both part of that same opponent-name series). Both
rounds: **250/250 sweep for sonnet-5** (sonnet-5 was Blue in both). Avg
final units round 1: us (Blue) ~29.56, opponent (Red) ~2.07 — consistent
with round 0's ~29.83/~2.01 reported in round 23's notes (~14x margin).
Twenty-fourth consecutive total sweep documented in this file (across 14
differently-named opponent identities). Grepped all `sim_*.txt` in
`/logs/rounds/1/` for `raceback|xception|panic` — zero hits.

### What I did this round
1. Confirmed `robot.py` has zero diff from `git show HEAD:robot.py` — no
   drift from the version described in rounds 4-23 above (per-unit soft
   targeting blending own-distance + target health + team
   coordination-distance + focus-bonus; opportunistic
   always-attack-if-adjacent, weakest-first; `direction_to` movement w/
   full 4-direction sidestep fallback; spawn-tile-escape when no enemies
   visible).
2. Sanity-checked it still runs clean and fast:
   `./rumblebot run term --results-only robot.py robot.py` (no
   exceptions, real combat, `Health 10 1 Units 2 1`) and vs
   `robot_v1_baseline.py` (no exceptions, `Health 32 16 Units 7 4`).
3. Used `tools/ab_test.py robot.py robot_v1_baseline.py --seeds 1-40
   --swap`: robot.py-as-Blue **26 wins / 12 losses / 2 ties**;
   robot.py-as-Red (swapped) **24 wins / 13 losses / 3 ties**. Combined:
   **50 wins / 25 losses / 5 ties** (~66.7% of decisive games) — same
   exact numbers as several previous rounds' checks (this A/B is
   deterministic given fixed seeds/bots, so identical results across
   rounds with no code changes is expected, not a coincidence). No
   regression.
4. **No code changes made this round.** Same reasoning as rounds 2-23: 24
   consecutive 250/250 sweeps across 14 different opponent identities,
   overwhelming (~14x this round) final-unit-count margins, zero runtime
   errors ever observed across ~6000+ simulated games total, and every
   tunable knob in the bot has already been explored (weight constants —
   settled no-effect, Rounds 10 & 12; overkill-avoidance — Round 6, mild
   negative; BFS-pathing — judged low-value, Round 5) with no robust
   improvement found. Validation-only round again — no evidence across 24
   rounds that opponents are close enough to competitive to justify
   regression risk from a large rewrite.

### Suggested next steps for future teammates
- Standing advice unchanged (24th time writing this): check
  `/logs/rounds/N/results.json` + final-unit-count margins FIRST thing
  next round, before making changes. Use `tools/ab_test.py` for any
  self-play A/B testing.
- Opponent-strength margin still bounces around by opponent identity
  (~4.6x-15x observed across rounds 19-24) with zero threat to our 100%
  round-win record across 24 rounds / 14 opponent identities so far.
- All three tunable weight constants remain settled/closed (Rounds 10 &
  12). Genuinely still-untried ideas unchanged: multi-step lookahead
  pathing (low-value per Round 5); "retreat when outnumbered" logic
  (untried, real regression risk, still not needed).
- `robot_v1_baseline.py` remains the frozen round-0 reference bot for A/B
  self-play testing — do not delete/modify it.

## Round 25 (this session — starting point was fresh /logs/rounds/0/ only, will produce /logs/rounds/1/)

**Status check (first thing, per standing advice, 25th time):** Only
`/logs/rounds/0/results.json` was present at session start. Opponent this
series is `jiricodes__jiricodes-bot` (15th differently-named opponent
identity across the rounds documented in this file). Result: **250/250
sweep for sonnet-5** (sonnet-5 was Red; `Blue wins 0, Red wins 250, ties 0`
via the standard win/loss snippet). Avg final units: opponent (Blue)
~0.796 (min 0, max 3), us (Red) ~30.92 (min 20, max 39). This is one of the
most lopsided margins recorded in this file's history (~38.8x) — opponent
averaged under 1 final unit across all 250 games, essentially fully wiped
out nearly every time. Twenty-fifth consecutive total sweep documented in
this file (across 15 differently-named opponent identities so far).
Grepped all 250 `sim_*.txt` for `raceback|xception|panic` — zero hits,
confirming clean execution across the entire series.
...
1. Confirmed `robot.py` has zero diff from `git show HEAD:robot.py`
   (`git status --short` clean at session start) — no drift from the
   version described in rounds 4-24 above (per-unit soft targeting
   blending own-distance + target health + team coordination-distance +
   focus-bonus; opportunistic always-attack-if-adjacent, weakest-first;
   `direction_to` movement w/ full 4-direction sidestep fallback;
   spawn-tile-escape when no enemies visible).
2. Sanity-checked it still runs clean and fast:
   - `./rumblebot run term --results-only robot.py robot.py` — no
     exceptions, real combat (`Health 25 12 Units 6 3`).
   - `./rumblebot run term --results-only robot.py robot_v1_baseline.py`
     — no exceptions, real combat (`Health 36 7 Units 10 2`).
3. Used `tools/ab_test.py robot.py robot_v1_baseline.py --seeds 1-40
   --swap`: robot.py-as-Blue **26 wins / 12 losses / 2 ties**;
   robot.py-as-Red (swapped) **24 wins / 13 losses / 3 ties**. Combined:
   **50 wins / 25 losses / 5 ties** (~66.7% of decisive games) — same
   exact numbers as several previous rounds' checks (deterministic given
   fixed seeds/bots with no code changes). No regression.
4. **No code changes made this round.** Same reasoning as rounds 2-24: 25
   consecutive 250/250 sweeps across 15 different opponent identities,
   overwhelming (~38.8x this round, the highest margin yet recorded)
   final-unit-count margins, zero runtime errors ever observed across
   ~6250+ simulated games total, and every tunable knob in the bot has
   already been explored (weight constants — settled no-effect, Rounds 10
   & 12; overkill-avoidance — Round 6, mild negative; BFS-pathing —
   judged low-value, Round 5) with no robust improvement found.
   Validation-only round again — no evidence across 25 rounds that
   opponents are close enough to competitive to justify regression risk
   from a large rewrite.

### Suggested next steps for future teammates
- Standing advice unchanged (25th time writing this): check
  `/logs/rounds/N/results.json` + final-unit-count margins FIRST thing
  next round, before making changes. Use `tools/ab_test.py` for any
  self-play A/B testing.
- Opponent-strength margin continues to bounce around wildly by opponent
  identity (~4.6x-38.8x observed across rounds 19-25) with zero threat to
  our 100% round-win record across 25 rounds / 15 opponent identities so
  far. This round's opponent (`jiricodes__jiricodes-bot`) was one of the
  weakest yet (avg <1 final unit).
- All three tunable weight constants remain settled/closed (Rounds 10 &
  12). Genuinely still-untried ideas unchanged: multi-step lookahead
  pathing (low-value per Round 5); "retreat when outnumbered" logic
  (untried, real regression risk, still not needed).
...
## Round 26 (this session — starting point had /logs/rounds/0/ AND /logs/rounds/1/
already present, will produce /logs/rounds/2/)

**Status check (first thing, per standing advice, 26th time):** Both
`/logs/rounds/0/results.json` and `/logs/rounds/1/results.json` were
already present at session start. Opponent this series is
`jiricodes__jiricodes-bot` (same name as round 25's notes above — round 0
and round 1 here are both part of that same opponent-name series). Both
rounds: **250/250 sweep for sonnet-5** (round 0: sonnet-5 Red; round 1:
sonnet-5 Blue). Recomputed round 1 directly: **250 wins / 0 losses / 0
ties**, avg final units us ~31.02, opponent ~0.9 (~34.5x margin) —
consistent with round 25's round-0 numbers (~30.9/~0.8, ~38.8x). Opponent
remains one of the weakest seen across this file's 15+ opponent
identities (near-total wipeout most games). Twenty-sixth consecutive
total sweep. Grepped all `sim_*.txt` in `/logs/rounds/1/` for
`raceback|xception|panic` — zero hits.

### What I did this round
1. Confirmed `robot.py` has zero diff from `git show HEAD:robot.py`
   (`git status --short` clean, `diff` empty) — no drift from the version
   described in rounds 4-25 above (per-unit soft targeting blending
   own-distance + target health + team coordination-distance +
   focus-bonus; opportunistic always-attack-if-adjacent, weakest-first;
   `direction_to` movement w/ full 4-direction sidestep fallback;
   spawn-tile-escape when no enemies visible).
2. Sanity-checked it still runs clean and fast (~0.95-1.0s wall-clock incl.
   setup, both `robot.py` vs `robot.py` and vs `robot_v1_baseline.py`, no
   exceptions, real combat both times).
3. Ran `tools/ab_test.py robot.py robot_v1_baseline.py --seeds 1-40
   --swap`: robot.py-as-Blue **26 wins / 12 losses / 2 ties**;
   robot.py-as-Red (swapped) **24 wins / 13 losses / 3 ties**. Combined:
   **50 wins / 25 losses / 5 ties** (~66.7% of decisive games) — identical
   to the numbers reported in rounds 22-25 (expected, since this A/B is
   fully deterministic given fixed seeds and unchanged bot code — same
   inputs, same outputs). No regression.
4. **No code changes made this round.** Same reasoning as rounds 2-25: 26
   consecutive 250/250 sweeps across 15 different opponent identities,
   overwhelming margins every time (this round ~34.5x), zero runtime
   errors ever observed across ~6500+ simulated games total, and every
   tunable knob in the bot has already been explored (weight constants —
   settled no-effect, Rounds 10 & 12; overkill-avoidance — Round 6, mild
   negative; BFS-pathing — judged low-value, Round 5) with no robust
   improvement found. Validation-only round again — the current opponent
   pool has never come remotely close to threatening our 100% round-win
   record, so the expected value of a risky rewrite remains strongly
   negative relative to a cheap validation pass.

### Suggested next steps for future teammates
- Standing advice unchanged (26th time writing this): check
  `/logs/rounds/N/results.json` + final-unit-count margins FIRST thing
  next round, before making changes. Use `tools/ab_test.py` for any
  self-play A/B testing (see Round 12 notes for usage/rationale).
- Opponent-strength margin continues to bounce around wildly by opponent
  identity (~4.6x-38.8x observed across rounds 19-26) with zero threat to
  our 100% round-win record across 26 rounds / 15 opponent identities so
  far. If this ever changes (opponent wins a round, or margin drops well
  below ~3x, or opponent avg final units climbs into double digits
  consistently across multiple rounds), that's the trigger to seriously
  invest in the still-untried ideas below instead of another
  validation-only round.
- All three tunable weight constants (`HEALTH_WEIGHT`, `FOCUS_BONUS`,
  `COORD_WEIGHT`) remain settled/closed from Rounds 10 & 12's large-sample
  tests (all ~50% i.e. no effect) — do not re-litigate without a
  fundamentally different targeting idea.
- Genuinely still-untried ideas (unchanged across ~26 rounds now):
  multi-step (2-3 move) lookahead pathing for escaping dead-ends near the
  map's wall corners (still low-value per Round 5's analysis); explicit
  "retreat when badly outnumbered locally" logic for individual low-health
  units (still untried, carries real regression risk against the current
  "always attack if adjacent" philosophy that keeps winning decisively).
  Neither has been necessary yet across 26 rounds and 15 opponent
  identities, all crushed by ~4.6x+ unit-count margins minimum.
- `robot_v1_baseline.py` remains the frozen round-0 reference bot for A/B
  self-play testing — do not delete/modify it. Still gives `robot.py` a
  consistent, reproducible, side-independent edge (this round: 50-25-5
...
## Round 27 (this session — starting point was fresh /logs/rounds/0/ only, will produce /logs/rounds/1/)

**Status check (first thing, per standing advice, 27th time):** Only
`/logs/rounds/0/results.json` was present at session start. Opponent this
series is `sbasu3__meek-bot` (16th differently-named opponent identity
across the rounds documented in this file). Result: **250/250 sweep for
sonnet-5** (sonnet-5 was Blue; `Blue wins 250, Red wins 0, ties 0` via the
standard win/loss snippet). Avg final units: us (Blue) ~14.75 (min 5, max
24), opponent (Red) ~1.92 (min 0, max 4) — ~7.7x margin, squarely in the
historical range (~4.6x-38.8x observed across rounds 19-26). Twenty-seventh
consecutive total sweep documented in this file (across 16 differently-
named opponent identities so far). Grepped all 250 `sim_*.txt` for
`raceback|xception|panic` — zero hits, confirming clean execution across
the entire series.
...
1. Confirmed `robot.py` has zero diff from `git show HEAD:robot.py`
   (`git status --short` clean, `diff` empty) — no drift from the version
   described in rounds 4-26 above (per-unit soft targeting blending
   own-distance + target health + team coordination-distance +
   focus-bonus; opportunistic always-attack-if-adjacent, weakest-first;
   `direction_to` movement w/ full 4-direction sidestep fallback;
   spawn-tile-escape when no enemies visible).
2. Sanity-checked it still runs clean and fast:
   `./rumblebot run term --results-only robot.py robot.py` (~0.76s
   wall-clock incl. ~90ms setup, no exceptions, real combat: `Health 26 10
   Units 6 3`) and vs `robot_v1_baseline.py` (~0.97s, no exceptions,
   `Health 7 32 Units 2 9`).
3. Ran `tools/ab_test.py robot.py robot_v1_baseline.py --seeds 1-40
   --swap`: robot.py-as-Blue **26 wins / 12 losses / 2 ties**;
   robot.py-as-Red (swapped) **24 wins / 13 losses / 3 ties**. Combined:
   **50 wins / 25 losses / 5 ties** (~66.7% of decisive games) — identical
   numbers to rounds 22-26 (expected: this A/B is fully deterministic
   given fixed seeds and unchanged bot code). No regression.
4. **No code changes made this round.** Same reasoning as rounds 2-26: 27
   consecutive 250/250 sweeps across 16 different opponent identities,
   overwhelming margins every time, zero runtime errors ever observed
   across ~6750+ simulated games total, and every tunable knob in the bot
   has already been explored (weight constants — settled no-effect,
   Rounds 10 & 12; overkill-avoidance — Round 6, mild negative; BFS-pathing
   — judged low-value, Round 5) with no robust improvement found.
   Validation-only round again.

### Suggested next steps for future teammates
- Standing advice unchanged (27th time writing this): check
  `/logs/rounds/N/results.json` + final-unit-count margins FIRST thing
  next round, before making changes. Use `tools/ab_test.py` for any
  self-play A/B testing (see Round 12 notes for usage/rationale).
- Opponent-strength margin continues to bounce around wildly by opponent
  identity (~4.6x-38.8x observed across rounds 19-27) with zero threat to
  our 100% round-win record across 27 rounds / 16 opponent identities so
  far. If this ever changes (opponent wins a round, or margin drops well
  below ~3x, or opponent avg final units climbs into double digits
  consistently across multiple rounds), that's the trigger to seriously
  invest in the still-untried ideas below instead of another
...
- All three tunable weight constants (`HEALTH_WEIGHT`, `FOCUS_BONUS`,
  ...
  tests (all ~50% i.e. no effect) — do not re-litigate without a
  fundamentally different targeting idea.
- Genuinely still-untried ideas (unchanged across ~27 rounds now):
  multi-step (2-3 move) lookahead pathing for escaping dead-ends near the
  map's wall corners (still low-value per Round 5's analysis); explicit
  "retreat when badly outnumbered locally" logic for individual low-health
  units (still untried, carries real regression risk against the current
  "always attack if adjacent" philosophy that keeps winning decisively).
  Neither has been necessary yet across 27 rounds and 16 opponent
  identities, all crushed by ~4.6x+ unit-count margins minimum.
- `robot_v1_baseline.py` remains the frozen round-0 reference bot for A/B
  self-play testing — do not delete/modify it. Still gives `robot.py` a
  consistent, reproducible, side-independent edge (this round: 50-25-5
...
## Round 28 (this session — starting point had /logs/rounds/0/ AND /logs/rounds/1/
already present, will produce /logs/rounds/2/)

**Status check (first thing, per standing advice, 28th time):** Both
`/logs/rounds/0/results.json` and `/logs/rounds/1/results.json` were
already present at session start. Opponent this series is
`sbasu3__meek-bot` (same name as round 27's notes above — round 0 and
round 1 here are both part of that same opponent-name series, matching
git log `Rung 15/58 (sbasu3__meek-bot, elo #44) — Round 1 Update`). Both
rounds: **250/250 sweep for sonnet-5** (sonnet-5 was Blue in both, per
`results.json`'s `details` field). Recomputed round 1 directly (standard
win/loss snippet + final-`Units` regex): **250 wins / 0 losses / 0 ties**,
avg final units us (Blue) **14.92**, opponent (Red) **1.92** (~7.8x
margin, range: us 6-27, opponent 0-5) — consistent with round 0's earlier
numbers reported in round 27's notes (~14.75/~1.92, ~7.7x). Twenty-eighth
consecutive total sweep documented in this file (across 16 differently-
named opponent identities so far — `sbasu3__meek-bot` is the same
identity as round 27, not a new one). Grepped all `sim_*.txt` in
`/logs/rounds/1/` for `raceback|xception|panic` — zero hits, confirming
clean execution.
...
1. Confirmed `robot.py` has zero diff from `git show HEAD:robot.py`
   (`git status --short` clean, `diff` empty) — no drift from the version
   described in rounds 4-27 above (per-unit soft targeting blending
   own-distance + target health + team coordination-distance +
   focus-bonus; opportunistic always-attack-if-adjacent, weakest-first;
   `direction_to` movement w/ full 4-direction sidestep fallback;
   spawn-tile-escape when no enemies visible).
2. Sanity-checked it still runs clean and fast:
   `./rumblebot run term --results-only robot.py robot.py` (~0.78s
   wall-clock incl. ~89ms setup, no exceptions, real combat: `Health 19 7
   Units 5 2`) and vs `robot_v1_baseline.py` (~0.72s, no exceptions,
   `Health 11 29 Units 4 6` — baseline won this particular single seed,
   expected occasional variance, see the larger A/B sample below for the
   real signal).
3. Ran `tools/ab_test.py robot.py robot_v1_baseline.py --seeds 1-40
   --swap`: robot.py-as-Blue **26 wins / 12 losses / 2 ties**;
   robot.py-as-Red (swapped) **24 wins / 13 losses / 3 ties**. Combined:
   **50 wins / 25 losses / 5 ties** (~66.7% of decisive games) — identical
   numbers to rounds 22-27 (expected: this A/B is fully deterministic
   given fixed seeds and unchanged bot code — same inputs, same outputs).
   No regression.
4. **No code changes made this round.** Same reasoning as rounds 2-27: 28
   consecutive 250/250 sweeps across 16 different opponent identities,
   overwhelming margins every time (this round ~7.8x, squarely in the
   historical ~4.6x-38.8x range), zero runtime errors ever observed across
   ~7000+ simulated games total, and every tunable knob in the bot has
   already been explored (weight constants — settled no-effect, Rounds 10
   & 12; overkill-avoidance — Round 6, mild negative; BFS-pathing — judged
   low-value, Round 5) with no robust improvement found. Validation-only
   round again — the current opponent pool has never come remotely close
   to threatening our 100% round-win record, so the expected value of a
   risky rewrite remains strongly negative relative to a cheap validation
   pass.

### Suggested next steps for future teammates
- Standing advice unchanged (28th time writing this): check
  `/logs/rounds/N/results.json` + final-unit-count margins FIRST thing
  next round, before making changes. Use `tools/ab_test.py` for any
  self-play A/B testing (see Round 12 notes for usage/rationale).
- Opponent-strength margin continues to bounce around wildly by opponent
  identity (~4.6x-38.8x observed across rounds 19-28) with zero threat to
  our 100% round-win record across 28 rounds / 16 opponent identities so
  far. If this ever changes (opponent wins a round, or margin drops well
  below ~3x, or opponent avg final units climbs into double digits
  consistently across multiple rounds), that's the trigger to seriously
  invest in the still-untried ideas below instead of another
...
- All three tunable weight constants (`HEALTH_WEIGHT`, `FOCUS_BONUS`,
  ...
  tests (all ~50% i.e. no effect) — do not re-litigate without a
  fundamentally different targeting idea.
- Genuinely still-untried ideas (unchanged across ~28 rounds now):
  multi-step (2-3 move) lookahead pathing for escaping dead-ends near the
  map's wall corners (still low-value per Round 5's analysis); explicit
  "retreat when badly outnumbered locally" logic for individual low-health
  units (still untried, carries real regression risk against the current
  "always attack if adjacent" philosophy that keeps winning decisively).
  Neither has been necessary yet across 28 rounds and 16 opponent
  identities, all crushed by ~4.6x+ unit-count margins minimum.
- `robot_v1_baseline.py` remains the frozen round-0 reference bot for A/B
  self-play testing — do not delete/modify it. Still gives `robot.py` a
  consistent, reproducible, side-independent edge (this round: 50-25-5
  combined across both sides, 40 seeds each direction, 80 games total —
  identical to previous rounds' numbers since nothing changed).
- **New suggestion for future teammates (finally breaking the pattern of
  repeating this advice unchanged)**: given ~28 rounds of validation-only
  sessions with a settled/closed set of "easy" tuning knobs, if you have
  spare step budget and want to actually try something new rather than
  re-validate, the two remaining untried ideas above (multi-step lookahead
  pathing, retreat-when-outnumbered logic) are the only genuinely
  unexplored levers left in this codebase. Both carry real regression
  risk against a bot that's winning every round, so only attempt them with
  a large-sample A/B (60+ seeds via `tools/ab_test.py`) BEFORE committing,
  not after — and be prepared to revert if the A/B doesn't show a clear,
  robust (not just "looks okay in 20 seeds") improvement over
  `robot_v1_baseline.py` and, ideally, over the current `robot.py` too
  (i.e. add a third A/B comparison: new-idea-variant vs current
  `robot.py`, not just vs the much older `robot_v1_baseline.py`).

## Round 29 (this session — starting point was fresh /logs/rounds/0/ only, will produce /logs/rounds/1/)

**Status check (first thing, per standing advice, 29th time):** Only
`/logs/rounds/0/results.json` was present at session start. Opponent this
series is `essickmango__fruity-test` (17th differently-named opponent
identity across the rounds documented in this file). Result: **250/250
sweep for sonnet-5** (sonnet-5 was Blue; `Blue wins 250, Red wins 0, ties
0` via the standard win/loss snippet). Avg final units: us (Blue) ~17.83
(min 10, max 27), opponent (Red) ~3.30 (min 0, max 7) — ~5.4x margin,
squarely in the historical range (~4.6x-38.8x observed across rounds
19-28). Twenty-ninth consecutive total sweep documented in this file
(across 17 differently-named opponent identities so far). Grepped all 250
`sim_*.txt` for `raceback|xception|panic` — zero hits, confirming clean
execution across the entire series.

### What I did this round
1. Confirmed `robot.py` has zero diff from `git show HEAD:robot.py`
   (`git status --short` clean, `diff` empty) — no drift from the version
   described in rounds 4-28 above (per-unit soft targeting blending
   own-distance + target health + team coordination-distance +
   focus-bonus; opportunistic always-attack-if-adjacent, weakest-first;
   `direction_to` movement w/ full 4-direction sidestep fallback;
   spawn-tile-escape when no enemies visible).
2. Sanity-checked it still runs clean and fast:
   `./rumblebot run term --results-only robot.py robot.py` (~0.98s
   wall-clock incl. ~90ms setup, no exceptions, real combat: `Health 21 0
   Units 5 0`) and vs `robot_v1_baseline.py` (~0.82s, no exceptions,
   `Health 33 11 Units 8 3`).
3. Ran `tools/ab_test.py robot.py robot_v1_baseline.py --seeds 1-40
   --swap`: robot.py-as-Blue **26 wins / 12 losses / 2 ties**;
   robot.py-as-Red (swapped) **24 wins / 13 losses / 3 ties**. Combined:
   **50 wins / 25 losses / 5 ties** (~66.7% of decisive games) — identical
   numbers to rounds 22-28 (expected: this A/B is fully deterministic
   given fixed seeds and unchanged bot code). No regression.
4. **No code changes made this round.** Same reasoning as rounds 2-28: 29
   consecutive 250/250 sweeps across 17 different opponent identities,
   overwhelming margins every time, zero runtime errors ever observed
   across ~7250+ simulated games total, and every tunable knob in the bot
   has already been explored (weight constants — settled no-effect,
   Rounds 10 & 12; overkill-avoidance — Round 6, mild negative; BFS-pathing
   — judged low-value, Round 5) with no robust improvement found.
   Validation-only round again — the current opponent pool has never come
   remotely close to threatening our 100% round-win record, so the
   expected value of a risky rewrite remains strongly negative relative
   to a cheap validation pass.

### Suggested next steps for future teammates
- Standing advice unchanged (29th time writing this): check
  `/logs/rounds/N/results.json` + final-unit-count margins FIRST thing
  next round, before making changes. Use `tools/ab_test.py` for any
  self-play A/B testing (see Round 12 notes for usage/rationale).
- Opponent-strength margin continues to bounce around wildly by opponent
  identity (~4.6x-38.8x observed across rounds 19-29) with zero threat to
  our 100% round-win record across 29 rounds / 17 opponent identities so
  far. If this ever changes (opponent wins a round, or margin drops well
  below ~3x, or opponent avg final units climbs into double digits
  consistently across multiple rounds), that's the trigger to seriously
  invest in the still-untried ideas below instead of another
...
- All three tunable weight constants (`HEALTH_WEIGHT`, `FOCUS_BONUS`,
  ...
  tests (all ~50% i.e. no effect) — do not re-litigate without a
  fundamentally different targeting idea.
- Genuinely still-untried ideas (unchanged across ~29 rounds now):
  multi-step (2-3 move) lookahead pathing for escaping dead-ends near the
  map's wall corners (still low-value per Round 5's analysis); explicit
  "retreat when badly outnumbered locally" logic for individual low-health
  units (still untried, carries real regression risk against the current
  "always attack if adjacent" philosophy that keeps winning decisively).
  Neither has been necessary yet across 29 rounds and 17 opponent
  identities, all crushed by ~4.6x+ unit-count margins minimum. If a
  future teammate has spare step budget and wants to try one of these,
  see Round 28's notes for the recommended A/B methodology (large-sample,
  compare against BOTH `robot_v1_baseline.py` AND current `robot.py`
  before committing).
- `robot_v1_baseline.py` remains the frozen round-0 reference bot for A/B
  self-play testing — do not delete/modify it. Still gives `robot.py` a
  consistent, reproducible, side-independent edge (this round: 50-25-5
