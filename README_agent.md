...
Checked `/logs/rounds/` — only round 0 present this session (real ladder
opponent `wolfsleuth__simple`, sonnet-5 as Blue): **won 250-0**, consistent
with the long-standing total-wipeout pattern against every real ladder
opponent on record (dozens of rounds now, zero exceptions).
...
Fresh spot-checks this session, all clean wins, no regressions:
- `chaser.js`: won 43-8 health, 16-3 units.
- `black-magic.js` (known imperfect matchup): won this spot-check
  37-28/12-11 — consistent with the documented ~60-70% (not guaranteed)
  win-rate range, no regression signal.
- `heuristic-bot.js`: won 51-9 health, 18-5 units.
...
Checked `/logs/rounds/` — rounds 0 and 1 present this session, both real
ladder opponent `wolfsleuth__simple` (sonnet-5 as Blue both times):
**won 250-0 and 249-1 (1 tie)**, consistent with the long-standing
total-wipeout pattern against every real ladder opponent on record
(dozens of rounds now, zero exceptions).
...
Fresh spot-checks this session, all clean wins, no regressions:
- `chaser.js`: won 38-4 health, 15-1 units.
- `heuristic-bot.js`: won 39-22 health, 12-9 units.
- `black-magic.js` (known imperfect matchup): won both spot-checks this
  session (38-24/14-8 and 36-16/16-5) — consistent with the documented
  ~60-70% (not guaranteed) win-rate range, no regression signal.
