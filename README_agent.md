...
## Round 2 (this session, continuing ketza__arthur matchup)
Opponent: `ketza__arthur` (continuing from Round 0-1, both already logged
in `/logs/rounds/0` and `/logs/rounds/1`, we were Red both times per
`details` field). Recomputed win/loss+avg-units for both:
- Round 0: Red (us) won 249/250, 0 losses, 1 tie. avg final units 18.85
  (us) vs 3.86 (opponent), ~4.88x margin.
- Round 1: Red (us) won 248/250, 2 losses, 0 ties. avg final units
  19.35 (us) vs 3.93 (opponent), ~4.93x margin.

Both rounds consistent with the "Round 1 (this session)" note already in
this file — near-total shutouts, margin ~4.9x, comfortably above the
~3x "still dominant" threshold from the recommended workflow. No time
to dig into the 2 losses this round beyond confirming (via the prior
Round 1 note's tie investigation, `sim_92.txt`) that the general pattern
here is "legitimately close symmetric games," not bugs.
...
confirming engine/harness unchanged). Ran `tools/ab_test.py robot.py
robot_v1_baseline.py --seeds 1-40 --swap --workers 16` → **40/40 both
as Blue and as Red** (unambiguous `{botname}_wins=N` labels) —
consistent with every post-retreat-adoption round's full-sweep finding,
no regression.

No local copy of `ketza__arthur`'s source found on disk (only log
directories, as with almost every previous opponent), so no way to
build/validate a targeted matchup-specific fix this session.

**No code changes made.** Rationale unchanged from the established
playbook: win rate remains near-total (497/500 games across both
logged rounds this matchup, 2 losses, 1 tie), margin (~4.88-4.93x) is
well above the ~3x "fully dominant" threshold, and — per the prior
session's final note — the "genuinely still-untried ideas" list is
already **empty** (multi-step BFS lookahead pathing and
outnumbered-retreat generalization were both tried and found neutral;
weight tuning is triply-closed). Confirm-and-stop remains the
lowest-risk/highest-EV action this round. Future teammates: continue
the validate-and-confirm workflow each round unless a future opponent's
round win rate drops below ~95% or margin drops persistently below
~2x, in which case a more fundamental architecture change (e.g. true
lookahead/minimax over predicted enemy moves) would be the next lever
to consider, per the prior round's closing note — not recommended
without a dedicated multi-round budget though.

## Round 1 (this session) — new opponent `mkap__test`

Opponent: `mkap__test` (new identity, ~38th distinct opponent seen),
logged as `/logs/rounds/0`. Result: **248/250 wins for sonnet-5** (we
were Red per `details`: "mkap__test was Blue and sonnet-5 was Red"), 0
losses, 2 ties. Avg final units: ~16.19 (us) vs ~5.13 (opponent),
~3.16x margin — near-total shutout, margin just above the ~3x "fully
dominant" threshold from the recommended workflow. `grep -li
"error|exception|traceback" sim_*.txt` → 0 matches across all 250 logs
(no crashes/exceptions). Investigated both non-wins (`sim_108.txt`,
`sim_147.txt`, both ties) — both ran the full 100 turns and ended in
genuinely close symmetric states (10v10 units/29v18hp; 8v8 units/
25v19hp) — no evidence of a bug, stuck/idle units, or wasted turns.
...
No local copy of `mkap__test`'s source found on disk (`find /
-iname "*mkap*"` outside `/logs/` → empty), same situation as almost
every previous opponent, so no way to build/validate a targeted
matchup-specific fix this session.

**No code changes made.** Rationale unchanged from the established
playbook: win rate is a near-total 99.2% (248/250, 0 losses, 2 ties),
margin (~3.16x) is just above the ~3x "fully dominant" threshold, both
non-win games reviewed are legitimately close symmetric ties (not
bugs), and — per prior sessions' final notes — the "genuinely
still-untried ideas" list is already **empty** (multi-step BFS
...
Future teammates: continue the validate-and-confirm workflow each
round unless a future opponent's round win rate drops below ~95% or
margin drops persistently below ~2x, in which case a more fundamental
architecture change (e.g. true lookahead/minimax over predicted enemy
moves) would be the next lever to consider — not recommended without a
dedicated multi-round budget though.

## Round 2 (this session, continuing mkap__test matchup)
Opponent: `mkap__test` (continuing from Round 0-1, both logged in
`/logs/rounds/0` and `/logs/rounds/1`, we were Red both times per
`details` field). Recomputed win/loss+avg-units for Round 1: Red (us)
won 249/250, 1 loss (Blue won `sim_156.txt`), 0 ties, avg final units
16.64 (us) vs 5.02 (opponent), ~3.32x margin — consistent with Round
0's ~3.16x (248/250, 0 losses, 2 ties per prior session's note). Both
rounds near-total shutouts, margin just above the ~3x "fully dominant"
threshold from the recommended workflow.

Investigated the one loss (`sim_156.txt`): ran the full 100 turns,
ended in a genuinely close symmetric state (8v7 units, 25v13 health) —
no evidence of a bug, stuck/idle units, or wasted turns; consistent
with every previous "close but legitimate" loss/tie pattern documented
elsewhere in this file. `grep -li "error|exception|traceback"` across
both rounds' sim logs (500 total) → 0 matches, no crashes/exceptions.
...
No local copy of `mkap__test`'s source found on disk (re-checked
`find / -iname "*mkap*"` outside `/logs/` → empty), same situation as
almost every previous opponent, so no way to build/validate a targeted
matchup-specific fix this session.

**No code changes made.** Rationale unchanged from the established
playbook: win rate remains near-total across both logged rounds this
matchup (497/500 games, 0 losses on Round 0, 1 loss on Round 1, 2 ties
on Round 0, 0 ties on Round 1), margin (~3.16-3.32x) remains just
above the ~3x "fully dominant" threshold, both non-win games reviewed
are legitimately close symmetric games/ties (not bugs), and — per
prior sessions' final notes — the "genuinely still-untried ideas" list
is already **empty** (multi-step BFS lookahead pathing and
outnumbered-retreat generalization have both been tried and found
neutral; weight tuning is triply-closed). Confirm-and-stop remains the
lowest-risk/highest-EV action this round. Future teammates: continue
the validate-and-confirm workflow each round unless a future
opponent's round win rate drops below ~95% or margin drops
persistently below ~2x, in which case a more fundamental architecture
