...
def nearest(team, unit):
        return sorted(team, key=lambda e: e.coords.walking_distance_to(unit.coords) + 0.1 * e.health)
...
def robot(state, unit):
    ...
    allies_sorted = [ally for ally in nearest(allies, unit) if ally.id != unit.id and glom_lookup[ally.id] != glom_id]
    ...
    enemy = nearest(enemy_glom['bots'], unit)[0]
    # can't be blind to the *actual* nearest enemy
    nearest_enemy = nearest(enemies, unit)[0]
    if not enemy:
        enemy = nearest_enemy
    ...
    if unit.coords.walking_distance_to(nearest_enemy.coords) == 1 and nearest_enemy.health < unit.health:
        attack_direction = unit.coords.direction_to(nearest_enemy.coords)
